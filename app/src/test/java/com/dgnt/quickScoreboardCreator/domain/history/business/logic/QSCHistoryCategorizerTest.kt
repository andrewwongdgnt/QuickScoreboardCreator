...
class QSCHistoryCategorizerTest {
    ...
    fun test2CategoriesWith2Entities() {
        ...
        sut(
            listOf(
                HistoryEntity(id = 1, title = "Basketball", icon = ScoreboardIcon.BASKETBALL, lastLookedAt = DateTime(2024, 1, 15, 12, 10), HistoricalScoreboardData(mapOf())),
                HistoryEntity(id = 2, title = "Spikeball", icon = ScoreboardIcon.BASKETBALL, lastLookedAt = DateTime(2024, 2, 14, 11, 0), HistoricalScoreboardData(mapOf())),
            )
        ).let {
    ...
    fun test2CategoriesWith4Entities() {
        ...
        sut(
            listOf(
                HistoryEntity(id = 1, title = "Basketball", icon = ScoreboardIcon.BASKETBALL, lastLookedAt = DateTime(2024, 1, 4, 12, 10), HistoricalScoreboardData(mapOf())),
                HistoryEntity(id = 2, title = "Spikeball", icon = ScoreboardIcon.BASKETBALL, lastLookedAt = DateTime(2024, 2, 16, 11, 0), HistoricalScoreboardData(mapOf())),
                HistoryEntity(id = 3, title = "Volleyball", icon = ScoreboardIcon.VOLLEYBALL, lastLookedAt = DateTime(2024, 1, 19, 12, 10), HistoricalScoreboardData(mapOf())),
                HistoryEntity(id = 4, title = "Tennis", icon = ScoreboardIcon.TENNIS, lastLookedAt = DateTime(2024, 2, 1, 5, 13), HistoricalScoreboardData(mapOf())),
            )
        ).let {
            Assert.assertEquals(2, it.size)
            Assert.assertEquals(DateTime(2024, 2, 1, 0,0), it[0].dateTime)
